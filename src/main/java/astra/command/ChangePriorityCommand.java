  ...
  */
  ...
  public boolean execute(ActivityList activities, Ui ui, Notebook notebook) {
    try {
        ...
        if (parts.length != 2) {
            throw new InputException("Argument missing. Use: changepriority <task number> /to <priority>");
        }
        ...
        if (tokens.length != 2) {
            throw new InputException("Task number or new priority missing. Use: changepriority <task number> /to <priority>");
        }
        ...
        if (newPriority <= 0 || newPriority > activities.getListSize()) {
            throw new InputException("Priority must be between 1 and " + activities.getListSize() + ".");
        }
        ...
        for (int i = 0; i < activities.getListSize(); i++) {
            if (i == (taskNumber - 1)) continue;
            Activity other = activities.getActivity(i);
            if (other instanceof Task) {
                Task t = (Task) other;
                if (oldPriority < newPriority && t.getPriority() > oldPriority && t.getPriority() <= newPriority) {
                    t.setPriority(t.getPriority() - 1);
                } else if (oldPriority > newPriority && t.getPriority() < oldPriority && t.getPriority() >= newPriority) {
                    t.setPriority(t.getPriority() + 1);
                }
            }
        }
